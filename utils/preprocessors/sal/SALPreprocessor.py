#!/usr/bin/env python3
# --------------------------------------
# Preprocessor for the Solver Interface Layer
# --------------------------------------
import sys, os
# --------------------------------------
from coreio import pp_warning, pp_error
# --------------------------------------
from SALConfig import SALConfiguration
# --------------------------------------
from SALTemplates import GenericTemplate, TemplateMap
# --------------------------------------
def main(args):
    config = SALConfiguration(args.config)
    config.ensure_consistency()
    GenericTemplate(args.output, args.template_directory, args.template,
                    TemplateDataMap[args.template](config)).render()
# --------------------------------------
if __name__ == '__main__':
    from argparse import ArgumentParser
    ap = ArgumentParser(description='GPiD Solver Interface Layer Preprocessor')

    ap.add_argument('config', metavar='<config-file>.yaml', type=str,
                    help='config-file to preprocess')

    ap.add_argument('-t', '--template', type=str, required=True,
                    help='preprocessor template guide')

    ap.add_argument('-d', '--template-directory', type=str, required=True,
                    help='template directory')

    ap.add_argument('-o', '--output', type=str, default='sal.cpp',
                    help='output file')

    args = ap.parse_args()
    try:
        main(args)
    except Exception as e:
        pp_error(e)
        sys.exit(1)
# --------------------------------------
