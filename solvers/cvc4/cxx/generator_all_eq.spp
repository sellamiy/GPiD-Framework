...
namespace gpid {
    ...
    void generateAbducibles_allEq(CVC4::ExprManager& ctx,
                                   CVC4Declarations& decls, HypothesesSet<CVC4Solver>& set) {
        std::vector<CVC4::Expr> knownVars;
        abduciblesUtils_allEq(ctx, decls, knownVars);

        /* Building abducibles */
        uint32_t pos = 0;
        alloc_gab<HypothesesSet<CVC4Solver>>(set.getSourceSize());
        for (uint32_t i = 0; i < knownVars.size(); i++) {
            for (uint32_t j = i + 1; j < knownVars.size(); j++) {
                ...
                store_gab_hyp<HypothesesSet<CVC4Solver>, CVC4::Expr>(set, pos, eq_expr);
                store_gab_hyp<HypothesesSet<CVC4Solver>, CVC4::Expr>(set, pos+1, neq_expr);
