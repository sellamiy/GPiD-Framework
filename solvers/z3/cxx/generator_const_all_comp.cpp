    ...
    void generateAbducibles_constAllComp(z3::context&, Z3Declarations& decls,
        ...
        for (uint32_t i = 0; i < vCount; i++) {
            for (uint32_t j = i+1; j < vCount; j++) {
                ...
                store_gab_hyp<LiteralsEngine<Z3SolverEngine>, z3::expr>(set, pos, cstl_gt);
                store_gab_hyp<LiteralsEngine<Z3SolverEngine>, z3::expr>(set, pos+1, cstl_ge);
                store_gab_hyp<LiteralsEngine<Z3SolverEngine>, z3::expr>(set, pos+2, cstl_lt);
                store_gab_hyp<LiteralsEngine<Z3SolverEngine>, z3::expr>(set, pos+3, cstl_le);
