...
namespace gpid {
    ...
    static inline
    void generateAbducibles_constAllEq(z3::context&, Z3Declarations& decls, HypothesesSet<Z3Solver>& set) {
        ...
        for (uint32_t i = 0; i < vCount; i++) {
            for (uint32_t j = i+1; j < vCount; j++) {
                ...
                store_gab_hyp<HypothesesSet<Z3Solver>, z3::expr>(set, pos, cstl_eq);
                store_gab_hyp<HypothesesSet<Z3Solver>, z3::expr>(set, pos+1, cstl_neq);
