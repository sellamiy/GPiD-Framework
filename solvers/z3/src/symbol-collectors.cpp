...
inline bool Z3Declarations::knows_id(unsigned id) const {
    return seen_ids.find(id) != seen_ids.end();
}

inline void Z3Declarations::collect_sort(z3::context& ctx, z3::sort s) {
...
inline void Z3Declarations::collect_fun(z3::context& ctx, z3::func_decl f) {
...
void Z3Declarations::collect(z3::expr e) {
    todo.push_back(e);
    while (!todo.empty()) {
