...
stream.close()
# --------------------
QFLIASet = core.ProblemSet()
stream = open('/home/mrcoco/QF_UFLIA.Sat.txt')
QFLIASet.add_file_list([ fn.strip() for fn in stream.readlines() if fn.strip != '' ])
stream.close()
# --------------------
# Script
# --------------------
...
script.add_variable('qf-uflia', QFLIASet)

for i in range(1,16):
    script.add_command(core.EvaluationCommand(3000+i, 'problems', 'gpid', 'first',
                                              [core.OptionData('timeout', { 1 : 10, 2 : 20 }),
                                               core.OptionData('size-limit', { 1 : i })]))
    script.add_command(core.EvaluationCommand(4000+i, 'qf-uflia', 'gpid', 'first',
                                              [core.OptionData('timeout', { 1 : 10, 2 : 20 }),
                                               core.OptionData('size-limit', { 1 : i })]))
script.add_command(core.EvaluationCommand(3000, 'problems', 'gpid', 'first',
                                              [core.OptionData('timeout', { 1 : 10, 2 : 20 })]))
script.add_command(core.EvaluationCommand(4000, 'qf-uflia', 'gpid', 'first',
                                              [core.OptionData('timeout', { 1 : 10, 2 : 20 })]))
